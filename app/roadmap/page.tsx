...
const ROADMAP: { group: string; items: Item[] }[] = [
  {
    ...
    items: [
      ...
      { title: 'ATS-risk badges on templates', detail: 'Every template card shows ✓ / △ / ✗ ATS with a tooltip — Sidebar/Modern/Tech flagged risky.', status: 'shipped' },
      { title: 'Cover letter as PDF cover page', detail: 'Export dialog checkbox prepends the cover letter as page 1 of the PDF.', status: 'shipped' },
      { title: 'JSON Resume import/export', detail: 'jsonresume.org schema — round-trip your resume in/out of any JSON Resume tool.', status: 'shipped' },
      { title: 'Share-link expiry', detail: 'Never / 24h / 7d / 30d. Expiry baked into the URL fragment; viewer renders "expired" past it.', status: 'shipped' },
      { title: 'Page-break preview guide', detail: 'Dashed line in the live preview at A4 boundaries; hidden in print.', status: 'shipped' },
      ...
      { title: 'Resume diff viewer', detail: 'Word-level LCS diff, inline or side-by-side.', status: 'shipped' },
    ],
  },
  {
    group: 'AI',
    items: [
      ...
      { title: 'JD-tailored rewrite', detail: 'Paste JD, AI rewrites summary + top bullets with diff preview. Auto-snapshots first.', status: 'shipped' },
      { title: 'Cover letter tone variants', detail: '4 tones: Professional, Formal, Casual, Concise. Per-tone temperature tuning.', status: 'shipped' },
      { title: 'ATS score trend', detail: 'SVG sparkline of score history with debounced snapshots.', status: 'shipped' },
    ],
  },
  {
    group: 'Infrastructure',
    items: [
      { title: 'Account deletion API', detail: 'GDPR-safe account deletion (user + profile cascade, R2 avatar cleanup).', status: 'shipped' },
      { title: 'Server-side rate limiting', detail: 'Enforce AI/PDF limits on the server. Kills bypass vector.', status: 'shipped' },
      { title: 'Upstash-backed rate limiting', detail: 'Per-IP/per-route limits shared across every Lambda when UPSTASH_REDIS_REST_URL is set. Falls back to in-memory.', status: 'shipped' },
      { title: 'Stripe webhook idempotency', detail: 'webhook_events table records every processed event.id; retries short-circuit before mutating state.', status: 'shipped' },
      { title: 'DB indexes for cron query paths', detail: 'profiles.last_seen_at, inactive_warned_at, notify_product, stripe_customer_id, user.created_at — no more full-table scans.', status: 'shipped' },
      { title: 'Inactive-account lifecycle', detail: 'Daily cron warns then deletes accounts inactive for 6 months with grace period.', status: 'shipped' },
      { title: 'R2 cleanup on avatar overwrite + delete', detail: 'Old avatars removed from Cloudflare R2 before a new upload, and the user prefix is wiped on account delete.', status: 'shipped' },
      { title: 'Schema-version migration scaffold', detail: 'Zustand persist version + migrate hook so future schema changes don\'t break existing localStorage payloads.', status: 'shipped' },
      { title: 'Stripe checkout for Pro tier', detail: 'Unlimited AI + PDF — signed webhooks, idempotent handlers.', status: 'shipped' },
      { title: 'Waitlist backend', detail: '/api/leads/waitlist with rate-limit + duplicate guard.', status: 'shipped' },
      ...
      { title: 'Bundled pdfjs worker', detail: 'Worker copied to /public/pdfjs/. Works behind corporate firewalls.', status: 'shipped' },
    ],
  },
  {
    group: 'Growth',
