...
- **Public write endpoints must validate server-side + rate-limit.** Don't trust client trimming. Use `await rateLimit()` + `clientId()` from `lib/rateLimit.ts` — the function is **async**; it backs onto Upstash Redis when `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` are set (real per-IP limit across every Lambda) and degrades to a per-process in-memory burst guard otherwise. Cap string lengths, validate email format, and parse JSON in a try/catch. `app/api/share/invite/route.ts` is the reference (same-origin check, auth, rate limit, HTML escaping).
...
- **Stripe webhook idempotency.** Every Stripe event id is recorded in the `webhook_events` table at the top of `app/api/stripe/webhook/route.ts` via `INSERT … ON CONFLICT DO NOTHING`. A replay short-circuits to `{ replay: true }` before any handler runs. Don't bypass this when adding new handlers — even if your handler is idempotent today, the next one might not be.
- **R2 cleanup on user changes.** Use `deleteFromR2(key)` / `deletePrefixFromR2(prefix)` from `lib/storage.ts` when overwriting an avatar (delete the old key) or deleting an account (delete `avatars/${userId}/`). Both no-op when R2 isn't configured and swallow errors so storage failures can't break the parent request.
