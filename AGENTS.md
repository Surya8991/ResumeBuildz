...
- **Cron & operator endpoints** are gated by a `CRON_SECRET` bearer, fail-closed (503 if unset, 401 on mismatch). See `app/api/cron/*` and `app/api/admin/broadcast`. Vercel injects `Authorization: Bearer $CRON_SECRET` for crons defined in `vercel.json`.
- **Marketing email needs consent + unsubscribe.** Only send to `profiles.notifyProduct = true`. Every marketing email must carry an unsubscribe link (`lib/emailTokens.ts` → stateless HMAC tokens, `/api/email/unsubscribe`). Transactional email (auth, receipts) does not get an unsubscribe footer.
